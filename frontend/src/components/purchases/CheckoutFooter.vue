<template>
    <span class="row fixed-bottom">
        <div class="container total-price-container border bg-primary text-light text-center">
            ...
            <br />
            <button class="btn btn-light shadow mx-2" v-on:click="$emit('resetButtonClick')">Reset</button>
            <button class="btn btn-success shadow mx-2" data-bs-toggle="modal" :data-bs-target="`#${MODAL_TAG_ID}`"
                v-on:click="$emit('checkoutButtonClick')">Checkout</button>
        </div>
    </span>
...
export default {
    ...
    setup(props) {
        ...
        const totalPrice = computed(() => props.purchaseLines.reduce((sum, line) => {
